...
type ContentType = 'stories' | 'essays' | 'poems' | 'articles' | 'research' | 'creative' | 'whatsapp' | 'conversations' | 'tutorials' | 'cultural' | 'multilingual';
...
const contentTypes: ContentTypeOption[] = [
  ...
  },
  {
    id: 'tutorials',
    label: 'Tutorials',
    description: 'How-to guides, step-by-step instructions, and learning materials',
    icon: <div className="w-6 h-6 bg-yellow-500 rounded-lg flex items-center justify-center">
  ...
  },
  {
    ...
    icon: <div className="w-6 h-6 bg-red-500 rounded-lg flex items-center justify-center">
...
export default function DocsPage() {
  const [selectedContentType, setSelectedContentType] = useState<ContentType>('stories');
  ...
  const handleFileSelect = (file: File) => {
    ...
    if (['whatsapp', 'conversations'].includes(selectedContentType)) {
      ...
      if (file.type !== 'text/plain' && !file.name.endsWith('.txt')) {
        setError(`Please select a .txt file for ${selectedContentType} exports`);
        return;
      }
      ...
      <div className="flex gap-8">
        ...
        <div className="w-80 flex-shrink-0">
          <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
            <h2 className="text-xl font-bold text-slate-900 mb-6">Content Types</h2>
        ...
        <div className="flex-1">
            ...
            {!uploadSuccess ? (
                  ...
                  {selectedFile ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        {['whatsapp', 'conversations'].includes(selectedContentType) && (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-slate-600 mb-8">
                  Your {selectedType?.label?.toLowerCase()} has been uploaded successfully. Thank you for contributing!
                </p>
                <button
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <p className="text-slate-600 text-sm">
                  {selectedContentType === 'stories' && 'Short stories: 1,000-7,500 words. Novellas: 7,500-40,000 words.'}
                  {selectedContentType === 'essays' && 'Personal essays: 500-2,000 words. Academic essays: 1,500-5,000 words.'}
                  {selectedContentType === 'poems' && 'Any length from haiku to epic poems. Focus on quality over quantity.'}
                  {selectedContentType === 'articles' && 'Feature articles: 1,000-3,000 words. How-to guides: 500-2,000 words.'}
                  {selectedContentType === 'research' && 'Research papers: 3,000-10,000 words. Include proper citations.'}
                  {selectedContentType === 'creative' && 'Experimental pieces: Any length. Be innovative and original.'}
                  {selectedContentType === 'whatsapp' && 'Chat exports: Any length. Include meaningful conversations and group discussions.'}
                  {selectedContentType === 'conversations' && 'Chat exports: Any length. Focus on diverse topics and natural language patterns.'}
                  {selectedContentType === 'tutorials' && 'Step-by-step guides: 500-5,000 words. Clear instructions with examples.'}
                  {selectedContentType === 'cultural' && 'Cultural content: Any length. Traditional knowledge, folklore, and expressions.'}
                  {selectedContentType === 'multilingual' && 'Multilingual content: Any length. Include language pairs and translations.'}
                </p>
              </div>
            ...
            {selectedContentType === 'whatsapp' && (
              <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                ...
                <p className="text-green-600 text-xs mt-2">
                  💡 Tip: Group chats with diverse topics and conversations are especially valuable for AI training!
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((item) => (
                <div key={item} className="flex items-center gap-4 p-4 bg-slate-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-slate-900">Sample {selectedType?.label?.slice(0, -1)} {item}</h4>
                    <p className="text-sm text-slate-600">Uploaded by User • 2 hours ago</p>
                  </div>
